...
from extra.utils import Matcher, BalancedPositiveNegativeSampler, roi_align
from extra.boxops import BoxCoder, box_iou, process_box, nms
...
class RoIHeads:
    ...
    def select_training_samples(self, proposal, target):
        ...
        proposal = Tensor.cat((proposal, gt_box))
        ...
        idx = Tensor.cat((pos_idx, neg_idx))
    ...
    def fastrcnn_inference(self, class_logit, box_regression, proposal, image_shape):
        ...
        for l in range(1, num_classes):
            ...
            label = Tensor.full((len(keep),), l, dtype=keep.dtype, device=device)
            
            boxes.append(box)
            labels.append(label)
            scores.append(score)

        results = dict(boxes=Tensor.cat(boxes), labels=Tensor.cat(labels), scores=Tensor.cat(scores))
        return results
        ...
        if self.has_mask():
            if self.training:
                ...
                if mask_proposal.shape[0] == 0:
                    losses.update(dict(roi_mask_loss=Tensor.tensor(0)))
                    return result, losses
            else:
                mask_proposal = result['boxes']
                
                if mask_proposal.shape[0] == 0:
                    result.update(dict(masks=Tensor.empty((0, 28, 28))))
                    return result, losses
